...
<?php 
    ...
    class Message {
        ...
        public function sendMessage($user_to, $body, $date){
            if ($body != "") {            
                $userLoggedIn = $this->user_obj->getUsername();
                $query = mysqli_query($this->con, "insert into messages (user_to, user_from, body, date, opened, viewed) values('$user_to','$userLoggedIn','$body','$date','no','no')") or die(mysqli_error($this->con));
            }
        ...
        public function getOtherChats(){
            ...
            foreach($chat as $username){
                ...
                if (is_null($last_msg_detail) || !$last_msg_detail) {
                    return "";
                }
                echo("<script>console.log('PHP: " . json_encode($last_msg_detail) . "');</script>");
